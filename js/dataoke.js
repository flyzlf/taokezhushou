console.log("hello,im dataoke.js");
...
var _TAOKEZHUSHOU=function(){
	...
	this.___Analysis=function(ele){
		var data={};
		data.srcurl=ele.find("a:first").attr("href");//商品ID页
		data.gid=ele.attr("data-itemid");
		data.title=$.trim(ele.find("img:first").attr("alt"));//商品标题
		data.price=ele.find(".quan_price span:eq(1)").text();//券后单价
		//data.imgurl=ele.find("img").attr("data-original");//图片地址
		data.yj=$.trim(ele.find(".jihua").prev().find("span:eq(1)").text()).replace("%","");//佣金
		data.coupondenomination=ele.find("div:first").next().next().find("span:first").text();//优惠券面额
		//data.coupondenomination=data.coupondenomination.indexOf("天")==-1?data.coupondenomination:0;
		data.sellcount=ele.find(".jihua").next().find("span:eq(1)").text();//总销量
		data.sitetype=ele.find(".quan_title").prev().attr("style").indexOf("tmall")==-1?"taobao":"tmall";//淘宝还是天猫
		return data;
	}
	this._AnalysisWenan=function(param){//分析文案页
		var div=$(param.html).find(".ju_mb_content");
		if(div.length==0){
		...
		tmp.imgurl=div.find("img").attr("src");//图片地址
...
$(function(){
	...
	$(".quan_add_u").after("<a href='#' class='explan' style='float:right;color:#fff;padding:2px;'>推广</a>").remove();
