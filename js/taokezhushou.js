console.log("hello,im taokezhushou.js");
var common=new _COMMON();
var _TAOKEZHUSHOU=function(){
	var that=this;
	this.href="http://www.taokezhushou.com";
	this.itemlist=[];
	...
	this._Analysis=function(param){//分析get回来的html,提取商品信息
		...
		if(param.html=="thispage"){
			_item=$(".goods2 .mg .good1_one");
			_item.length==0?_item=$(".goods1 .mg .good1_one"):true;
		}
		else
			_item=$(param.html).find(".goods2 .mg .good1_one");
		var ret=[];
		if(_item.length==0){
	...
	this.___Analysis=function(ele){
		var data={};
		data.srcurl="http://www.taokezhushou.com/"+ele.find("a:first").attr("href");//商品ID页
		data.title=$.trim(ele.find("img").attr("alt"));//商品标题
		data.price=ele.find(".btm1 span").text();//券后单价
		//data.imgurl=ele.find("img").attr("data-original");//图片地址
		data.yj=ele.find(".commission .com1 span").text().replace("%","");//佣金
		data.coupondenomination=ele.find(".coupon .num1").text();//优惠券面额
		//data.coupondenomination=data.coupondenomination.indexOf("天")==-1?data.coupondenomination:0;
		data.sellcount=ele.find(".commission .com4 span").text();//总销量
		data.sitetype=ele.find(".title .tit1").length==0?"taobao":"tmall";//淘宝还是天猫
		return data;
	}
	this._AnalysisWenan=function(param){//分析文案页
		var div=$(param.html).find("#wenan");
		if(div.length==0){
		...
		tmp.couponurl=div.find("a:not(:last)").after("AAcouponurlAA").attr("href");//优惠券地址
		tmp.imgurl=div.find("img").attr("copy-src");//图片地址
	...
	this.createTopNav=function(){//创建导航条
		var html='\
			<div class="container-fluid navbar-fixed-top" style="position: fixed;right: 0;left: 0;top:0;z-index: 1030;background-color: #bce8f1;height:42px">\
				<div class="panel panel-info" style="border-color: #bce8f1;">\
					<div class="panel-heading ">\
						<div class="btn-toolbar">\
							<div class="btn-group input-group">\
								<button type="button" style="height: 40px;background: #009f95;border: none;padding: 10px;color: white;" class="btn btn btn-info -autosendthispage">自动发本页</button>\
								<button type="button" style="height: 40px;background: #d9534f;border: none;padding: 10px;color: white;"  class="btn btn btn-danger -autosendstop">停止发送</button>\
								<button type="button" style="height: 40px;background: #009f95;border: none;padding: 10px;color: white;"  class="btn btn btn-info -sendRecordListclear">清空发送记录</button>\
								...
								<em style="height: 40px;border: none;padding: 20px;"  class="btn btn  -msger">准备完毕</em>\
							</div>\
						</div>\
					</div>\
				</div>\
